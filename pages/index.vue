<template>
  <div>
    ...
    >
      ...
      <div
        class="flex items-center justify-end m-4 space-x-2 flex-1 text-white"
      >
        <span>Language:</span>
        <select
          v-model="locale"
...
const { data: links }: { data: Ref<RepoType[]> } = await useAsyncData(
  "repos",
  () => $fetch("/api/repos?lang=" + locale.value),
  { watch: [locale] },
);
const { data: experiences }: { data: Ref<ExpType[]> } = await useAsyncData(
  "experiences",
  () => $fetch("/api/experiences?lang=" + locale.value),
  { watch: [locale] },
);
</script>
